...
const QuotaBanner: React.FC<QuotaBannerProps> = ({ open, type, onClose }) => {
  ...
  const title =
    type === "migration"
      ? "سقف انتقال محصولات شما پر شده است"
      : "سقف بروزرسانی لحظه‌ای شما پر شده است";
  const desc =
    type === "migration"
        ...
        <div className="mt-2 rounded-xl border border-orange-200 bg-gradient-to-r from-orange-50 to-red-50 shadow-sm">
          <div className="p-4 flex items-start justify-between gap-4">
            <div className="flex-1">
              <h3 className="text-sm sm:text-base font-bold text-orange-700 mb-1">
                {title}
              </h3>
              <p className="text-xs sm:text-sm text-orange-700/90">
                {desc} می‌توانید از طریق دکمه زیر به صفحه قیمت‌گذاری بروید و پلن
