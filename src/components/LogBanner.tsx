...
export default function LogBanner({ logs, onOpenLink }: Props) {
  ...
  return (
    <div
      className="mt-4 bg-white/80 backdrop-blur border border-amber-200 rounded-xl shadow p-3"
      dir="rtl"
    >
      <div className="flex items-center justify-between mb-2">
        <div className="text-sm font-semibold text-amber-700">
          گزارش خطاها و اعلان‌ها
        </div>
      ...
      <div className="max-h-48 overflow-y-auto space-y-2 pr-1">
        {logs.slice(0, 10).map((log) => (
          <div
            key={log.id}
            className="flex items-start justify-between gap-3 p-2 rounded-lg border border-amber-100 bg-amber-50/60"
          >
            <div className="flex-1">
              ...
              <div className="text-[12px] text-gray-500 mt-1">
                ...
                <span className="">
                  زمان: {new Date(log.ts).toLocaleTimeString()}
                </span>
              </div>
            </div>
            {onOpenLink && (
