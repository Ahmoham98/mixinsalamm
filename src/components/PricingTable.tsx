...
const PricingTable: React.FC<PricingTableProps> = ({
  ...
  const getPlanGradient = (planName: string, isCurrent: boolean) => {
    ...
    switch (planName.toLowerCase()) {
      case "free":
        return "bg-gradient-to-br from-[#95ed2f] to-[#30cfb7]";
      case "basic":
        return "bg-gradient-to-br from-[#95ed2f] to-[#30cfb7]";
      case "pro":
        return "bg-gradient-to-br from-[#95ed2f] to-[#30cfb7]";
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {plans.map((plan) => {
        ...
        const migrationQuota =
          (plan as any).quota_migration ?? plan.quota_migrations ?? 0;
        ...
        return (
          <div
            ...
            className={`relative rounded-3xl overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-2xl ${
              isCurrent
                ? "shadow-2xl ring-2 bg-gradient-to-r from-[#95ed2f] to-[#30cfb7] ring-opacity-50"
                : "shadow-lg hover:shadow-xl"
            }`}
          >
            {/* Background */}
            <div
              className={`absolute inset-0 ${getPlanBgColor(plan.name, isCurrent)} `}
            />
            ...
            <div className="relative p-6 flex flex-col h-full">
              ...
              <div className="text-center mb-6">
                <div
                  className={`inline-flex items-center justify-center w-16 h-16 rounded-full mb-4 ${getPlanGradient(plan.name, isCurrent)}`}
                >
              ...
              <div className="flex-1 space-y-3 mb-6">
                {/* Migration Quota */}
                <div
                  className={`flex items-center justify-between p-3 rounded-xl border ${getPlanBorderColor(plan.name, isCurrent)} bg-white/50 backdrop-blur-sm`}
                >
