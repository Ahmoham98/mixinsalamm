...
export const mixinApi = {
  validateCredentials: async (
    url: string,
    token: string,
  ): Promise<MixinValidationResponse> => {
    try {
      console.log("Validating Mixin credentials:", { url, token });

      const response = await api.post<MixinValidationResponse>(
        `/mixin/client/?mixin_url=${encodeURIComponent(url)}&token=${encodeURIComponent(token)}`,
      );
      ...
      if (error.response?.status === 403) {
        ...
        throw new Error(
          "some error occurred... could be from server or from our request.",
        );
      }
      throw error;
    }
  ...
  createProduct: async (
    ...
    try {
      const response = await api.post("/products/create/mixin", productData, {
        headers: {
          Authorization: `Bearer ${credentials.access_token}`,
        },
        params: {
      ...
      if (error.response?.data) {
        throw new Error(
          error.response.data.detail || "Failed to create Mixin product",
        );
      }
  ...
  getProductImages: async (
    ...
    try {
      ...
      if (response.data?.result && Array.isArray(response.data.result)) {
        ...
        items.sort(
          (a: any, b: any) => (b?.default ? 1 : 0) - (a?.default ? 1 : 0),
        );
    ...
    try {
      ...
      while (page <= maxPages) {
        ...
        const totalPages = Number(
          response.data?.total_pages || response.data?.total_page,
        );
