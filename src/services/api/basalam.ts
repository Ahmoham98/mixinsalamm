...
export const basalamApi = {
  ...
  getProducts: async (
    ...
    try {
      ...
      console.log(
        "Full Request URL:",
        `${BASE_URL}/products/my-basalam-products/${vendorId}`,
      );
      ...
      if (error instanceof AxiosError && error.response) {
        ...
        if (error.response.status === 404) {
          console.error(
            "Product endpoint not found. Please check the API endpoint.",
          );
        } else if (error.response.status === 401) {
          console.error("Unauthorized. Please check your access token.");
        } else if (error.response.status === 403) {
          console.error(
            "Forbidden. You may not have permission to access these products.",
          );
        }
      }
      return [];
  ...
  uploadImage: async (
    ...
    try {
      ...
      const response = await api.post(
        `/products/sync-image?image_url=${encodeURIComponent(imageUrl)}`,
        null,
        {
          headers: {
            Authorization: `Bearer ${credentials.access_token}`,
      ...
      throw new Error(
        `Failed to upload image to Basalam: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  },
  ...
  createProduct: async (
    ...
    try {
      const response = await api.post(
        `/products/create/basalam/${vendorId}`,
        productData,
        {
          headers: {
            Authorization: `Bearer ${credentials.access_token}`,
  ...
  getCategoryUnitType: async (
    credentials: BasalamCredentials,
    categoryId: number,
  ): Promise<{ unit_type: { id: number; title: string } | null } | null> => {
    try {
      const response = await api.get(
        `/products/category-unit-type?category_id=${categoryId}`,
        {
          headers: {
            Authorization: `Bearer ${credentials.access_token}`,
          },
