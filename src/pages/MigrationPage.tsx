...
export default function MigrationPage() {
  ...
  const uniqueBasalamProducts = useProductsStore(
    (s) => s.uniqueBasalamProducts,
  );
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const vendorId =
    (useAuthStore.getState()?.basalamCredentials as any)?.vendor?.id ||
    undefined;
    ...
    const loadAll = async () => {
      ...
      try {
        ...
        const [mxRes, bsRes] = await Promise.all([
          fetch(
            `${import.meta.env.VITE_API_BASE_URL || "https://mixinsalam-backend.liara.run"}/products/my-mixin-products/all?mixin_url=${mixinUrlParam}`,
            {
              headers: {
                Authorization: `Bearer ${mixinCredentials.access_token}`,
              },
            },
          ),
          fetch(
            `${import.meta.env.VITE_API_BASE_URL || "https://mixinsalam-backend.liara.run"}/products/my-basalam-products/${vendorId}/all`,
            {
              headers: {
        ...
        setGlobalMixinProducts(
          Array.isArray(mxData?.products) ? mxData.products : [],
        );
        setGlobalBasalamProducts(
          Array.isArray(bsData?.products) ? bsData.products : [],
        );
      } catch (e: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#5b9fdb]/10 to-[#ff6040]/10">
      <header className="sticky top-0 bg-white/70 backdrop-blur-md shadow-sm z-10 border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-8 py-5 flex items-center justify-between">
          <h1 className="text-2xl font-bold bg-gradient-to-r from-[#30cfb7] to-[#ffa454] bg-clip-text text-transparent">
      ...
      <main className="max-w-7xl mx-auto px-8 py-8">
        <BackHomeButton />
        <section
          className="bg-gradient-to-r from-[#30cfb7]/15 to-[#ffa454]/15 border border-[#30cfb7]/20 rounded-2xl p-6 shadow-md mb-8"
          dir="rtl"
        >
          <div className="flex items-center gap-4">
            ...
            <div className="text-right">
              ...
              <p className="text-gray-600 text-sm">
                لیست‌های کامل محصولات میکسین و باسلام بارگذاری می‌شود و آماده
                انتقال خواهند بود.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
            <div className="bg-white/80 backdrop-blur-md border border-gray-100 rounded-xl p-6 shadow-md">
              <h3 className="text-lg font-semibold text-gray-800 mb-4 text-right">
                محصولات منحصر به میکسین ({uniqueMixinProducts.length})
              </h3>
              <div className="max-h-[400px] overflow-y-auto pr-2">
                ...
                {uniqueMixinProducts.length > 100 && (
                  <div className="text-xs text-gray-500 mt-2 text-right">
                    ... و {uniqueMixinProducts.length - 100} محصول دیگر
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-white/80 backdrop-blur-md border border-gray-100 rounded-xl p-6 shadow-md">
              <h3 className="text-lg font-semibold text-gray-800 mb-4 text-right">
                محصولات منحصر به باسلام ({uniqueBasalamProducts.length})
              </h3>
              <div className="max-h-[400px] overflow-y-auto pr-2">
                ...
                {uniqueBasalamProducts.length > 100 && (
                  <div className="text-xs text-gray-500 mt-2 text-right">
                    ... و {uniqueBasalamProducts.length - 100} محصول دیگر
                  </div>
                )}
              </div>
            </div>
